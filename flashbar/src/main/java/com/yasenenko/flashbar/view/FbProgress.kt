...
class FbProgress : View {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            width = viewWidth.coerceAtMost(widthSize)
        } else {
            width = viewWidth
        }
        ...
        if (heightMode == MeasureSpec.EXACTLY || widthMode == MeasureSpec.EXACTLY) {
            ...
            height = viewHeight.coerceAtMost(heightSize)
        } else {
            height = viewHeight
        }
        ...
        if (isSpinning) {
            ...
            if (!linearProgress) {
                ...
                offset = (1.0f - Math.pow(
                    (1.0f - this.progress / 360.0f).toDouble(), (2.0f * factor).toDouble()
                )).toFloat() * 360.0f
                progress = (1.0f - Math.pow(
                    (1.0f - this.progress / 360.0f).toDouble(), factor.toDouble()
                )).toFloat() * 360.0f
            }
    ...
    private fun setAnimationEnabled() {
        val animationValue: Float = Settings.Global.getFloat(
    ...
    private fun setupBounds(layoutWidth: Int, layoutHeight: Int) {
        ...
        if (!fillRadius) {
            val minValue =
                (layoutWidth - paddingLeft - paddingRight).coerceAtMost(layoutHeight - paddingBottom - paddingTop)
    ...
    private fun parseAttributes(a: TypedArray) {
        ...
        circleRadius = a.getDimension(
            R.styleable.FbProgress_fbp_circleRadius, circleRadius.toFloat()
        ).toInt()
        ...
        barSpinCycleTime = a.getInt(
            R.styleable.FbProgress_fbp_barSpinCycleTime, barSpinCycleTime.toInt()
        ).toDouble()
